...
tm_list = open("./tm_int_testjson.list") #get TM path
for tm in tm_list:
    if "#" not in tm:
        tm_path = tm.strip()
        tm_name = "tp03" + "_" + "002" + "_" + re.findall('._(.+?)$', tm)[0] #name of output file
        ...
        with open("../02_INT_CHK/%s/test.json" %tm_path) as input:
            tm_file = open("./int_python_pat_testjson/%s.py" %tm_name, "w")
            tm_file.write('\n')
            tm_file.write('###############################\n')
            tm_file.write('# TM : %s  \n' %tm_path)
            tm_file.write('###############################\n')
            ...
            for element in fixed_mem:
                ...
                if "sections" in element:
                    ...
                    with open("../02_INT_CHK/%s/set_mem.txt" %tm_path) as des_mem:
            ...
            with open("../02_INT_CHK/%s/set_reg.txt" %tm_path) as reg_info: #get data of reg setting
            ...
            if (dir == "08_DMR_random") or (dir == "08_DMR_SPMC"):
                ...
                result = open("../02_INT_CHK/%s/result.txt.l" %tm_path)
